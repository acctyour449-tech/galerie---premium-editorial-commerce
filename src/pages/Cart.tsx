    ...
    <div className="max-w-screen-2xl mx-auto px-8 py-16 grid grid-cols-1 lg:grid-cols-12 gap-16 items-start animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <section className="lg:col-span-8 flex flex-col gap-12">
        <header className="mb-4">
          ...
          <p className="text-on-surface-variant font-medium mt-4 italic">3 items in your curation.</p>
        </header>

        <div className="flex flex-col gap-8">
          {/* Mock Items */}
          {[PRODUCTS[0], PRODUCTS[1]].map((product, idx) => (
            <motion.div 
              key={product.id}
            ...
            >
              <div className="w-40 h-52 bg-surface-container-low rounded-lg overflow-hidden flex-shrink-0 shadow-inner">
                <img alt={product.name} className="w-full h-full object-cover mix-blend-multiply transition-transform group-hover:scale-105" src={product.images[0]} referrerPolicy="no-referrer" />
              </div>
              <div className="flex flex-col flex-grow justify-between py-2">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-2xl font-semibold text-on-surface mb-1">{product.name}</h3>
                    <p className="text-[10px] text-on-surface-variant font-bold uppercase tracking-[0.2em]">{product.category} • {product.material}</p>
                  </div>
                  <button className="text-on-surface-variant hover:text-error transition-colors p-2 -mr-2 -mt-2 rounded-full hover:bg-error/10">
                    <X size={20} />
                  </button>
                </div>
                <div className="flex justify-between items-end mt-6">
                  ...
                  <div className="text-right">
                    <p className="text-xl font-semibold text-on-surface">${product.price}.00</p>
                  </div>
                </div>
              </div>
            </motion.div>
        ...
        >
          ...
          <div className="flex flex-col gap-6 text-sm">
            <div className="flex justify-between text-on-surface-variant">
              ...
              <span className="font-bold text-on-surface">$1,030.00</span>
            </div>
            <div className="flex justify-between text-on-surface-variant">
          ...
          <div className="flex justify-between items-end">
            ...
            <span className="text-4xl font-bold text-on-surface tracking-tighter">$1,030.00</span>
          </div>
